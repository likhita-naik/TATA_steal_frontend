...
@Component({
  selector: "app-pneumatic",
  templateUrl: "./pneumatic.component.html",
  styleUrls: ["./pneumatic.component.css"],
})
export class PneumaticComponent implements OnInit, AfterViewInit, OnDestroy {
  IP: any = "";
  ID: any = "";
  canvas: any;
  currentArea: any = "";
  ...
  @ViewChild("NameModal", { static: false }) NameModal: TemplateRef<any>;
  ...
  OnAddingHydroPanel() {
    ...
    this.canvas.remove(this.hydroPanel), this.canvas.add(tempCanvas);
    this.canvas.renderAll();
    pointsString =
      cords.tl.x.toString() +
      ";" +
    ...
    temp = {
      ...
      roi_data: {
        ...
        flasher_status: null,
        panel_key_id: ++this.hydro_key_id,
        roi_data: {
          panel_no: this.hydroRoiName.value,
          bbox: pointsString,
          panel_key_id: ++this.hydro_key_id,
  ...
  SaveAddedHydroRoi() {
    ...
    roiData.push(
      this.hydrolicROIData[this.hydrolicROIData.length - 1].roi_data
    );
    ...
    var cameraData = {
      ...
      panel_id:
        this.hydrolicROIData[this.hydrolicROIData.length - 1].roi_data.panel_id,
      panel_key_id:
        this.hydrolicROIData[this.hydrolicROIData.length - 1].roi_data.roi_data
          .panel_key_id,
      roi_data:
        this.hydrolicROIData[this.hydrolicROIData.length - 1].roi_data.roi_data,
    };
    ...
    this.server.AddHydraulicROI(body).subscribe((response: any) => {
      ...
      if (response.success) {
        ...
        this.canvas.remove(
          this.hydrolicROIData[this.hydrolicROIData.length - 1].roi_canvas
        );
        this.canvas.remove(
          this.hydrolicROIData[this.hydrolicROIData.length - 1].roi_name_canvas
        );
  ...
  DrawExistPneuPanels() {
    ...
    this.hydro_key_id = 0;
    this.PanelData.pneumatic_data.forEach((data: any, i: number) => {
      console.log(data);
      ...
      var tempObj = {
        ...
        roi_data: {
          ...
          roi_data: {
            panel_no: data.panel_no,
            bbox: data.bbox,
            panel_key_id: data.panel_key_id,
            unallocated_job_status: data.unallocated_job_status,
          },
        },
      };
      this.pneu_key_id < data.panel_key_id
        ? (this.pneu_key_id = data.panel_key_id)
        : "";
      console.log(this.pneu_key_id);
      this.pnuematicData.push(tempObj);
  ...
  DeletePanel() {
    ...
    this.DeletePneuROI(this.selectedEditId);
  }
  DeletePneuROI(id: number) {
    this.selectedEditId = id;
    this.selectedId = id;
    var tempRoiCanvas = this.pnuematicData[id].roi_canvas;
    var tempTextCanvas = this.pnuematicData[id].roi_name_canvas;
    console.log(id);
    console.log(
      "roi data",
      this.pnuematicData[this.selectedEditId].roi_data.roi_data
    );
    // this.allPanelData.splice(id, 1)
    var body = {
      id: this.ID,
      panel_key_id:
        this.pnuematicData[this.selectedId].roi_data.roi_data.panel_key_id,
      imagename: this.imageName,
    };
    this.server.DeletePneuRoi(body).subscribe((response: any) => {
      this.server.notification(response.message);
      if (response.success) {
        this.pnuematicData.splice(id, 1);
    ...
    temp = {
      ...
      roi_data: {
        ...
        roi_data: {
          panel_no: this.hydroRoiName.value,
          bbox: pointsString,
          panel_key_id: ++this.pneu_key_id,
          unallocated_job_status: this.unallocatedJobStatus ? true : false,
        },
      },
    };
    this.pnuematicData.push(temp);
    this.hydroRoiName.setValue(null);
    this.hydroRoiName.updateValueAndValidity();
    this.hydroRoiName.reset();
    this.hydroRoiName.addValidators(Validators.required);
    console.log(temp);
    this.SaveAddedPnueRoi();
  }
  ngOnDestroy(): void {}
  SaveAddedPnueRoi() {
    var roiData: any[] = [];
    ...
    console.log(this.pnuematicData[this.pnuematicData.length - 1].roi_data);

    var rw_points: any[] = [];

    roiData.push(this.pnuematicData[this.pnuematicData.length - 1].roi_data);
    ...
    var cameraData = {
      ...
      image_name: this.imageName,
      panel_id:
        this.pnuematicData[this.pnuematicData.length - 1].roi_data.panel_id,
      panel_key_id:
        this.pnuematicData[this.pnuematicData.length - 1].roi_data.roi_data
          .panel_key_id,
      roi_data:
        this.pnuematicData[this.pnuematicData.length - 1].roi_data.roi_data,
    };
    ...
    this.server.AddPnuematicROI(body).subscribe(
      (response: any) => {
        console.log(response);
        if (response.success) {
          this.modalService.dismissAll();
          ...
          this.canvas.remove(
            this.pnuematicData[this.pnuematicData.length - 1].roi_canvas
          );
          this.canvas.remove(
            this.pnuematicData[this.pnuematicData.length - 1].roi_name_canvas
          );
          this.pnuematicData.splice(this.pnuematicData.length - 1, 1);
          this.pneu_key_id--;
        }
        this.server.notification(response.message);
      },
