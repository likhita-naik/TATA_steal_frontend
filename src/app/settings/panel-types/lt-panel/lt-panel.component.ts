...
@Component({
  ...
  styleUrls: ['./lt-panel.component.css']
})
...
ngAfterViewInit(): void {
  ...
  this.canvas.on('object:moving', (options: any) => {
    ...
    if (this.iseditRW) {
      ...
      //this.allPanelData[this.selectedPanelforRW].rw_canvas.calcCoords()
...
GetPanelPoints() {
  ...
  this.PanelData.panel_data.forEach((points: any, id: number) => {
   ...
   console.log(this.panel_key_id,'panel_key_id')
...
FilterUnallocated(panelsData:any[]){
  // console.log(panelData.roi_data.roi_data.unallocated_job_status,'unplanned  data')
  return  panelsData.filter((panelData:any)=> panelData.roi_data.roi_data.unallocated_job_status ==true)
...
FilterAllocated(panelsData:any){
  ...
  return  panelsData.filter((panelData:any)=> panelData.roi_data.roi_data.unallocated_job_status == false)
}
