...
export class JobSheetUploadComponent implements OnInit, OnDestroy {
  ...
  ekUpload() {
    ...
    var parseFile = (file: any) => {
      var fileName = this.excelFile? this.excelFile.name.replace(/ /g, '_'):''
 ...
 SampleExcelDownload() {
    ...
    this.Server.SampleExcelDownload().subscribe(
      (response: any) => {
        ...
        var fileName = "SAMPLE_EXCEL" + " " + (this.Server.dateTransformbyPattern(new Date, 'yyyy_MM_dd_HH_mm_ss')) + '.xlsx'
