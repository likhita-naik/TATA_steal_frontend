...
import crypto, { format } from "crypto-js";
...
export class ESIMoniterComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  editField: string;
  intervalTime: number;
  rackProcess: FormControl = new FormControl();
  selectedRiro: any;
  Images: any[] = [];
  intervalforData: any;
  deleteJob: any;
  ...
  unplannedPrevLiveCount: number = 0;
  ...
  isShowUnplanned: FormGroup = new FormGroup({
    notification:new FormControl(true),
    alertSound:new FormControl(true)
  });
  selectedJobType: any;
  jobTypeSelected: any;
  violationDetails: any[] = [];
  flasherLiveLog: Observable<any[]> = of([]);
  rtspLiveIp: Observable<any>;
  riHeaderCount: number = 5;
  roHeaderCount: number = 5;
  queryParams: any;
  tagnames: any[] = [];
  filterTagname: any;
  ...
  l1Data: Observable<any>;
  tempL1data: any;
  totalL1data: Observable<number> = of(0);
  isHistory: Boolean = false;
  showDataHistory:boolean=false
  ...
  dataHeaders: any[] = [
    ...
    { item_text: "Isolating Locations", item_id: 6 },
    { item_text: "Switch Board Name", item_id: 27 },
    ...
    { item_text: "Magnetic Sticker", item_id: 17 },
    ...
    { item_text: "Rack History", item_id: 30 },
  ...
  modalOpen(modal: any, rtsp?: any, job?: any) {
   ...
   this.selectedJob?  (this.selectedJob.riro_data= this.selectedJob.riro_data.filter((item:any)=>( item!==null))):''
    rtsp ? (this.rtspLiveIp = rtsp) : "";
    this.isImgLoading = true;

    this.modalService
      .open(modal, { size: "xl", fullscreen: false,scrollable:true})
      .result.then(
        (result) => {
          this.AddCameraForm.reset();
          this.tempAddedJob = null;
  ...
  getFilteredL1Data(event: any) {
    var tempL1data = { ...this.tempL1data };
    if (this.filterTagname != null) {
      tempL1data.l1data = tempL1data.l1data.filter(
        (element: any) => element.tagname == this.filterTagname.data
      );

      this.l1Data = of(tempL1data);
    } else {
      this.l1Data = of(this.tempL1data);
    }
  }

  L1ModalOpen(modal: any, size: any) {
    this.tagnames = [];
    this.server.GetL1data().subscribe(
      (response: any) => {
        if (response.success) {
          this.tempL1data = response.message;
          this.l1Data = of(response.message);
          response.message.l1data.forEach((element: any) => {
            this.tagnames.push({
      ...
      (Err) => {
        this.server.notification("Error while fetching the L1 data", "Retry");
      }
    );
  }
  ...
  DeleteSelectedfield() {
    ...
    if (this.editField == "board") {
      ...
      console.log('delete department feild ',data1)
  ...
  ShowLog(modal: any, data: any) {
    ...
    this.modalService.dismissAll();
    this.server
      .GetFlasherDetails(data.data.ip_address, data.data.panel_data.panel_id)
      .subscribe(
        (response: any) => {
          container.classList.remove("loading");
          if (response.success) {
  ...
  getFlasherLiveLog(modal: any) {
    this.modalService.dismissAll();
    this.server.GetFlasherLog().subscribe(
      (response: any) => {
        if (response.success) {
          this.flasherLiveLog = of(response.message);
          this.modalService.open(modal, {
            size: "xl",
            backdrop: "static",
            centered: true,
  ...
  GetUnplannedData() {
    this.unplannedInterval = setInterval(
      () => {
        this.server.GetUnAllocatedCount().subscribe((response: any) => {
          if (response.success) {
            var diff = response.now_live_count - this.previouslivecount;

            if (diff > 0) {
              for (let i = 0; i < diff; i++)
                setTimeout(() => {
                  this.unallocatedJob = true;
                  this.unallocatedJobData = response.message[i];
                  if (this.isShowUnplanned.get('notification').value) {
                    this.ShowUJobAlert();
                    this.isShowUnplanned.get('alertSound').value? this.alertSound():''
                  } 
                }, 100);
            }
            this.previouslivecount = response.now_live_count;
          }
        });
      },
      this.isActive
  ...
  // ToUnplannedPage(){
  //   var link = this.router.serializeUrl(this.router.createUrlTree(['app/jobsheetMoniter/ESI_unallocated'],{queryParams:{isHistory:false,shutdownName:this.shutdownInfo.shutdownname,time:this.shutdownInfo.timestamp,}} ))
  //             window.open(link, '_blank')

  // }
  ToUnplannedPage() {
    var link = this.router.serializeUrl(
      this.router.createUrlTree(["app/ESI_unallocated"])
    );
    window.open(link, "_blank");
  }
  ToggleShowdataHistory(){
   this.showDataHistory=!this.showDataHistory
  }
  ...
  get getAddedValueControls() {
    return this.manualDataAddForm.get("addedValues") as FormArray;
  }
  addNewValue() {
