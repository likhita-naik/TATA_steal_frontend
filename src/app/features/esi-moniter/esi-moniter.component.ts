...
export class ESIMoniterComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  isShowUnplanned: FormControl = new FormControl(true, Validators.requiredTrue);
  selectedJobType: any;
  jobTypeSelected: any;
  ...
  mainJobForm: FormGroup = new FormGroup({
    ...
    no_of_isolating_points: new FormControl("", Validators.required),
    isolating_location: new FormControl("", Validators.required),
    relayIp: new FormControl(""),
  });
  ...
  ResetJobsheetModal(modal: any) {
    this.modalService.open(modal, { backdrop: "static", centered: true,scrollable:true });
  }
  ResetJobSheet() {
            ...
            if (diff > 0) {
              for (let i = 0; i < diff; i++)
                setTimeout(() => {
                  ...
                  if (this.isShowUnplanned.value) {
                    this.ShowUJobAlert();
                    this.alertSound();
                  } else {
                    console.log("this is from the unplanned jobs popup");
                  }
                }, 100);
            }
