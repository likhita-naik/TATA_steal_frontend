  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  ngOnInit(): void {
    ...
    if (!this.latest || this.isLatest) {
      this.webServer.LiveCCViolationData(this.selectedDepartment,this.selectedCameraId).subscribe(
        (Rdata: any) => {
  ...
  Submit() {
    ...
    this.interval2 = setInterval(() => {
      if (this.isdate) {
        ...
        this.webServer.LiveCCViolationData(this.selectedDepartment,this.selectedCameraId).subscribe((Rdata: any) => {
          if (Rdata.success) {
            var cviol = Rdata.message;
            var response = { ...Rdata };
